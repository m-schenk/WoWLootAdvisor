...
exports.addPlayer = (req, res, next) => {
    ...
    player
        ...
        .catch((err) => {
            ...
            error.message =
                "Failed to save new player in database (controllers/player addPlayer())";
...
exports.getPlayerById = (req, res, next) => {
    ...
    Player.findOne({ id: playerId })
        ...
        .catch((err) => {
            const error = new Error(err);
            error.message =
                "Failed to fetch player from database (controllers/player getPlayerById())";
            error.httpStatusCode = 500;
            return next(error);
        });
};
...
exports.getPlayerSelf = (req, res, next) => {
    Player.find({ id: req.session.playerId })
        ...
        .catch((err) => {
            const error = new Error(err);
            error.message =
                "Failed to fetch player from database (controllers/player getPlayerSelf())";
            error.httpStatusCode = 500;
            return next(error);
        });
};
